...
const media=['hero-nanofixac-steam.webp','hero-services.webp','hero-faq-reviews.webp','hero-contact.webp','chemical-pre-treatment.webp','chemical-steam-process.webp','in-situ-steam-cleaning.webp','comparison-focus.webp','steam-coil-cleaning.webp','aircon-cleaning-sequence.gif','case-drainage-cleaning.webp','case-in-situ-steam-cleaning.webp','commercial-steam-cleaning.webp','protected-workflow.webp','post-clean-inspection.webp','pricing-consultation.webp','faq-knowledge.webp'];
...
function renderMedia(){$('#media-grid').innerHTML=media.map(file=>`<article class="media-card"><img src="/assets/images/${file}" alt="${esc(file)}" loading="lazy"><div><strong>${esc(file)}</strong><small>Generated site asset · ${file.endsWith('.gif')?'GIF animation':'image'}</small></div></article>`).join('')}
